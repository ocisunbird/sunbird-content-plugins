...
MCQController.imagegrid.getOptionsTemplate = function (question) {
    ...
    <% _.each(question.data.options,function(val,key){ %>\
        <div role="radio" aria-checked="" tabindex="0" class="option-width-50 option-block org-ekstep-questionunit-mcq-option-element<% if(val.isCorrect) { %> mcq-correct-answer<% } %>" tabindex="0" onclick="MCQController.onSelectingOption(this, <%= key %>);MCQController.horizontal.onOptionSelected(event,<%= key %>)">\
            ...
            <% } %>\
            <div class="option-text-container-mcq mcq-imagegrid-text" style="width: 70vw;max-width: 65%;left: 29%;font-weight: bold;max-height: 50%;position: absolute;text-overflow: ellipsis;top: 40%">\
          <%  if(val.text) { %>\
              <span><%= val.text %></span>\
          <% } %>\
            </div>\
